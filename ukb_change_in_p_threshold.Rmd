...
max_len <- max(sapply(final_gene_list, length))

final_gene_df <- as.data.frame(
  lapply(final_gene_list, function(x) {
    c(x, rep(NA, max_len - length(x)))
  })
)

write.csv(final_gene_df, file = "./data/processed/threshold_6_genes_annotated_mapped_ukb.csv", row.names = FALSE)
```
