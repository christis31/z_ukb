...
plink2 --bfile ukb_cal_allChrs --exclude snp_failed_arraybatch_qc.txt --maf 0.01 --mac 100 --geno 0.01 --hwe 1e-15 0.001 midp --mind 0.1 --write-snplist --write-samples --no-id-header --out qc_pass



```

Needed to also filter out samples excluded based on sex, kinship, ancestry from UKB provided files 
```{r}
...
#one line version 
regenie --step 1 --bed ukb_cal_allChrs --extract qc_pass.snplist --keep qc_pass.id --phenoFile filter_bc.txt --covarFile covariates_breast.txt --bt --bsize 1000 --lowmem --lowmem-prefix tmpdir/regenie_tmp_preds --out ukb_step1_BT
```
