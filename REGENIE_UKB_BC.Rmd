...
./regenie \
  --step 1 \
  --bed qc_pass\
  --keep #sample QS either breast or lung #is this needed since we are deleting their phenotypic things anyway
  --keep #LD prune in
  --phenoFile bc.txt \
  --phenoColList ... # i think we can have more than one phenotype column here?
  --remove fid_to_remove_breast.txt \ #sample QS either breast or lung 
...
regenie --step 1 --bed qc_pass --extract qc_pass_ld09_full.prune.in --remove fid_to_remove_breast.txt --phenoFile pheno_breast.txt --phenoColList bbd_pl_pros,bbd_npl_pros,bbd_bn_pros,dcis_pros,lcis_pros,bc --covarFile covariates.txt --covarColList PC1,PC2,PC3,PC4,PC5,PC6,PC7,PC8,PC9,PC10 --bt --bsize 1000 --lowmem --lowmem-prefix tmpdir/regenie_tmp_preds --out ukb_step1_BT --loocv

#maybe also remove ICLD, low-complexity stuff 
```
...
--pred ukb_step1_BT_lung_never_pred.list
#SO MANY DIFFERENT SETTINGS WHICH I DONT KNOW WHAT VALUE TO GIVE -- EXAMPLE: MAC, threshold, minINFO
#WHAT MEMORY SETTING TO USE IN EACH COMMAND? 
...
```{r}
#Step 2 shoudl be with mem2_ssd2_v2_x4
```
